...
def guidance_context(
    *,
    ...
    return {
        ...
        "target_role": _text(job_title or (job_json or {}).get("title") or source.get("expected_job_title")),
...
def build_interview_action_plan(
    *,
    ...
    target_skill = (
        context["shared_skills"] or context["target_skills"] or context["resume_skills"] or ["目标岗位能力"]
    )[0]
    basis = _compact(answer_corpus, 180) or _compact(anchor["detail"], 180) or f"你已提供的「{anchor_name}」经历"
    ...
    if "candidate_action" in gaps:
        actions.append(_action(
            ...
            success_criteria=["出现清晰的个人动作", "说明与团队的边界", "能追问到具体过程而不矛盾"],
            priority=1,
            effort="15 分钟",
        ))
    ...
    for index, issue in enumerate(issues[:3], start=1):
        ...
        profiles = {
            "method_and_tradeoff": {
                ...
                "criteria": ["补齐“方案—约束—取舍”三项", "不强行添加数字，也不新增未经用户提供的事实"],
            },
            "outcome_expression": {
                "why": (
            ...
            "relevance_alignment": {
                ...
                "criteria": ["形成一条“岗位要求→真实动作”的对应关系", "只重排已有事实，不把岗位要求写成个人经历"],
            },
            "career_narrative": {
                "why": (
                ...
                "criteria": ["用“过去积累—转向触发—当前行动”三句讲清路径", "意愿与已具备能力明确区分"],
            },
            "role_clarity": {
                "why": (
                ...
                "criteria": ["简介包含目标角色、能力和经历证据", "不包含地址、电话、教育日期等基本信息"],
            },
            ...
            "skill_or_experience_building": {
                ...
                "criteria": ["完成一个与岗位要求直接相关的任务", "完成后新增经历 Claim 与可查看产出"],
            },
            "evidence_strengthening": {
                ...
                "criteria": ["Claim 已由用户确认并补齐上下文", "至少一种来源可追溯，且未扩大授权用途"],
            },
        }
        ...
        if profile is None:
            profile = {
                "why": (
                    f"「{diagnosis}」与「{anchor}」直接相关；本项只处理这一处，"
                    "不要求重写整份简历。"
                ),
                "first_step": recommended.get("why") or "先核对真实经历，再决定改写或转为实践任务。",
                "criteria": (
