        ...
        if json_match:
            ...
            try:
                ...
                for k, v in data.items():
                    ...
                    converted[eng_key] = v
                # 对嵌套字段做简单适配
                if "work_experience" in converted and isinstance(converted["work_experience"], list):
                    new_work = []
                    for exp in converted["work_experience"]:
                        if isinstance(exp, dict):
                            ...
                            for ek, ev in exp.items():
                                if ek == "公司": exp_mapped["company"] = ev
                                elif ek == "职位": exp_mapped["position"] = ev
                                elif ek == "时间段": exp_mapped["duration"] = ev  # 需要额外处理
                                elif ek == "工作内容": exp_mapped["description"] = "\n".join(ev) if isinstance(ev, list) else ev
                ...
                if "skills" in converted and isinstance(converted["skills"], list):
                    # 转换为 {name, level} 列表
                    converted["skills"] = [{"name": s, "level": "intermediate"} for s in converted["skills"]]
