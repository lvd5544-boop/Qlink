/** 评分规则说明（通俗语言，供用户点击「当前」「可提升空间」时查看） */
...
const V2_CRITERIA = [
  ...
  { label: '成长空间', pct: 10, desc: '基于可观察经历的学习与发展线索，不是录用预测' },
];
...
export function getPotentialScoreRules() {
  return {
    title: '「可提升空间」是怎么算的？',
    intro: '这是一个规则内参考值，表示在假设补齐缺失技能和软实力后，匹配分可能变化的范围。',
    points: [
      '以你当前的评分为基础',
      '假设你把岗位要求的缺失技能都补上',
      '假设软实力方面的差距也补齐',
    ...
    note: '它不代表个人潜力、面试概率或录用结果；新的“可提升空间模拟”会逐项展示假设和证据门槛。',
    disclaimer: SCORE_DISCLAIMER,
  };
}
